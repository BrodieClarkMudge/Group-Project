...
Animal::Animal(Texture2D texture, string type, string resource, int mHealth,
               int mHunger, int mThirst, int consHunger, int consThirst,
               int prodInt, Sound soundFX) {
  species = type;
  sprite = texture;
  health = mHealth;
  maxHealth = mHealth;
...
string Animal::getState() {
  ...
  oss << species << ", HP: " << health << ", Hunger: " << hunger
      << ", Thirst: " << thirst << (isAlive() ? ", Alive" : ", Dead");
  return oss.str();
}

void Animal::drink() { thirst = maxThirst; }
...
bool Animal::isAlive() { return isAliveState; }

string Animal::getSpecies() { return species; }

string Animal::getResourceType() { return producesResource; }
...
bool Animal::hasResourceAvailable() { return hasResourceReady; }

void Animal::setTexture(Texture2D tex) { sprite = tex; }

void Animal::draw(float x, float y, float width, float height) {
  if (isAliveState) {
    DrawTexturePro(sprite,
                   {0, 0, static_cast<float>(sprite.width),
                    static_cast<float>(sprite.height)},  // source rect
...
void Animal::makeSound() { PlaySound(soundEffect); }
...
int Animal::getThirst() { return thirst; }
