...
// globals
  ...
  while (!WindowShouldClose()) {
    ...
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
      Vector2 mouse = GetMousePosition();
      for (auto& t : tiles) {
        if (CheckCollisionPointRec(mouse, t.getRect()) && ui.hoeing == false &&
            watering == false && t.getType() == TileType::HOED) {
          if (t.hasCrop()) {
      ...
      for (auto& t : tiles) {
        ...
        if (t.getType() == TileType::YARD && t.hasAnimal()) {
          ...
          if (need > 0 && water > 0) {
            ...
            // INPUT VALIDATION
            if (give > water) {
              give = water;  // don't give more water than we have to avoid
                             // negative
            }
    ...
    */
    ...
    for (auto& t : tiles) {
      ...
      */
      ...
      if (t.hasCrop()) {
        ...
        if (t.getCropTimer() >= growEvery &&
            t.getCrop()->getWaterAmount() > 0) {
          t.getCrop()->Grow();
          t.setCropTimer(0.0f);
        }
    ...
    DrawText(
        "5:Tomato(50)  6:Potato(80)  7:Pumpkin(120)  LMB:Plant/Harvest  S:Save "
        " L:Load",
        20, weatherBarY + 35, 18, RAYWHITE);
    ...
    if (ui.hoeing == true && ui.coins >= 5 && shop.getPlacing() == false) {
      DrawTexturePro(tex.getHoeTex(),
                     ...
                     Vector2{80 / 2.0f, 80 / 2.0f}, 0.0f, WHITE);
      for (auto& t : tiles) {
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) &&
            t.getType() == TileType::GRASS &&
    ...
    if (watering) {
      DrawTexturePro(
          waterCanTex,
          {0, 0, (float)waterCanTex.width, (float)waterCanTex.height},
          {mousePos.x, mousePos.y, 80, 80}, {80 / 2.0f, 80 / 2.0f}, 0.0f,
          WHITE);
    }
