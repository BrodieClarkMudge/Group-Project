...
int main() {
  ...
  RebuildGrid(tiles, gridCols, gridRows, areaWidth, gridHeight, offsetX,
              offsetY);
  ...
  while (!WindowShouldClose()) {
    ...
    if (dayTimer >= dayLength) {
      ...
      currentDay++;  // update your displayed day too
      dayTimer = 0.0f;
    }
    ...
    // Grid controls
    // ----------------------
    if (shop.getExpanding() == true && gridRows < maxGrid &&
        gridCols < maxGrid) {
      gridRows += 2;
      gridCols += 3;
    ...
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
      ...
      for (auto& tile : tiles) {
        ...
        if (CheckCollisionPointRec(mouse, tile.rect) && hoeing == false &&
            watering == false && tile.type == TileType::HOED) {
          ...
          if (tile.crop) {
            ...
            if (tile.crop->IsMature()) {
              ...
              if (remove)
                tile.crop.reset();  // crops marked for deletion are removed
              tile.cropTimer =
                  0.0f;  // reset growth timer (so regen crops are reset)
            }
            ...
            switch (selectedCrop) {
              case SelectedCrop::BERRY:
                tile.crop = std::make_unique<Berry>(
                    0.0);  // maybe remove this param from class?
                break;
              case SelectedCrop::POTATO:
                tile.crop = std::make_unique<Potato>(0.0);
                break;
              case SelectedCrop::PUMPKIN:
                tile.crop = std::make_unique<Pumpkin>(0.0);
                break;
            }
    ...
    if (!shop.getOpen() && !shop.getSelectedItem().empty()) {
      ...
      for (auto& tile : tiles) {
        if (CheckCollisionPointRec(mouse, tile.rect)) {
          if (shop.getSelectedItem() == "YARD" &&
              coins >= 20 & tile.type != TileType::YARD) {
            tile.type = TileType::YARD;
            coins -= 20;
          } else if (shop.getSelectedItem() == "COW" && coins >= 50 &&
                     tile.type == TileType::YARD && !tile.animal) {
            ...
            coins -= 40;
                     ...
                     tile.type == TileType::YARD && !tile.animal) {
            tile.animal =
                std::make_unique<Chicken>(tex.chickenTex, tex.chickenSound);
            coins -= 30;
    ...
    DrawTexturePro(
        tex.hoeTex,
        Rectangle{0, 0, (float)tex.hoeTex.width, (float)tex.hoeTex.height}, hoe,
    ...
    DrawTexturePro(tex.shopTex,
                   Rectangle{-10, -10, (float)tex.shopTex.width + 20,
                             (float)tex.shopTex.height + 20},
    ...
    for (auto& tile : tiles) {
      if (tile.type == TileType::GRASS) {
        DrawTexturePro(tex.grassTex,
                       Rectangle{0, 0, (float)tex.grassTex.width,
                                 (float)tex.grassTex.height},
                       tile.rect, Vector2{0, 0}, 0.0f, WHITE);
      } else if (tile.type == TileType::YARD) {
        DrawTexturePro(tex.yardTex,
                       Rectangle{0, 0, (float)tex.yardTex.width,
                                 (float)tex.yardTex.height},
                       tile.rect, Vector2{0, 0}, 0.0f, WHITE);
      } else if (tile.type == TileType::HOED) {
        DrawTexturePro(tex.hoedTex,
                       Rectangle{0, 0, (float)tex.hoedTex.width,
                                 (float)tex.hoedTex.height},
                       tile.rect, Vector2{0, 0}, 0.0f, WHITE);
      }
      ...
      if (tile.crop && tile.crop->getWaterAmount() != 0) {
        Rectangle drop = {tile.rect.x + tile.rect.width - 16, tile.rect.y + 6,
                          10, 10};
      ...
      if (tile.crop) {
        ...
        // Optional: Add weather-based growth effects
        std::string currentWeather = weatherSystem.getCurrentWeather();

        // Fog slows growth by 20%
        if (currentWeather == "Fog") {
          growthMultiplier *= 0.8f;
        }
        // Heatwave boosts growth slightly more (optional)
        if (currentWeather == "Heatwave") {
          growthMultiplier *= 1.1f;
        }

        // Base grow time (e.g., 5s per stage), then divide by multiplier
        // to speed up growth
        float growEvery = 5.0f / growthMultiplier;

        // Only grow if watered and timer met
        if (tile.cropTimer >= growEvery && tile.crop->getWaterAmount() > 0) {
          tile.crop->Grow();
          tile.cropTimer = 0.0f;
        }

        // Handle withering (e.g. if not harvested)
        if (tile.crop->IsWithering() && tile.crop->IsMature()) {
          tile.ripeTimer += dt;
          if (tile.ripeTimer >= 5.0f) {
            tile.crop.reset();  // Remove crop
            continue;
          }
        } else {
          tile.ripeTimer = 0.0f;
      ...
      DrawText(
          "1:Cow  2:Sheep  3:Chicken  4:Pig  LMB:Place/Remove, 5:Berry  "
          "6:Potato  7:Pumpkin",
          20, winHeight - 60, 20, RAYWHITE);
      ...
      for (auto& tile : tiles) {
        if (tile.animal && CheckCollisionPointRec(mousePos, tile.rect)) {
          ...
          DrawRectangle(tooltipX - 4, tooltipY - 4, textWidth + 8,
                        textHeight + 8, Fade(BLACK, 0.8f));
      ...
      if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) &&
          CheckCollisionPointRec(mousePos, optionsBackground) &&
          ui.hoeing == false && shop.getOpen() == false &&
          shop.getPlacing() == false) {
        option.ShowOptions(option);
      }

      if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) &&
          CheckCollisionPointRec(mousePos, shopUI) &&
          option.getOptionsOpen() == false && ui.hoeing == false) {
        shop.Open();
      }

      shop.Update(tiles, coins, tex.yardTex, tex.cowTex, tex.sheepTex,
                  tex.chickenTex, tex.pigTex);
      shop.Draw();

      if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) &&
          CheckCollisionPointRec(mousePos, hoe) &&
          option.getOptionsOpen() == false && shop.getPlacing() == false &&
          shop.getOpen() == false) {
        ui.hoeing = !ui.hoeing;
      }
      ...
      if (hoeing == true) {
        if (ui.hoeing == true && coins >= 5 && shop.getPlacing() == false) {
          DrawTexturePro(tex.hoeTex,
                         Rectangle{0, 0, (float)tex.hoeTex.width,
                                   (float)tex.hoeTex.height},
                         Rectangle{mousePos.x, mousePos.y, 80, 80},
                         Vector2{80 / 2.0f, 80 / 2.0f}, 0.0, WHITE);
        }

        for (auto& tile : tiles) {
          if (hoeing && IsMouseButtonPressed(MOUSE_LEFT_BUTTON) &&
        ...
        if (watering) {
          DrawTexturePro(
              waterCanTex,
              {0, 0, (float)waterCanTex.width, (float)waterCanTex.height},
              {mousePos.x, mousePos.y, 80, 80}, {80 / 2.0f, 80 / 2.0f}, 0.0,
              WHITE);
        }
      }
      ...
      if (shop.getPlacing()) {
        ...
        DrawText("PLACING - to exit ENTER", (int)placingShop.x + 10,
                 (int)placingShop.y + 10, 20, DARKPURPLE);
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) &&
            CheckCollisionPointRec(mousePos, placingShop)) {
          shop.setPlacingFalse();
        }
      }

      // Paste this inside the existing BeginDrawing() block of your farm
      // main.cpp AFTER the farm grid is drawn, but BEFORE EndDrawing()

      // --- Weather Panel at bottom of screen ---
      int panelX = offsetX;
      int panelY = winHeight - 210;  // place above the white instructions
      int panelW = areaWidth;
      int panelH = 180;

      Color bg = {235, 240, 245, 255};
      Color panelColor = {255, 255, 255, 255};
      ...
      DrawRectangleRounded(
          {(float)panelX + 4, (float)panelY + 4, (float)panelW, (float)panelH},
          0.12f, 20, shadow);
      DrawRectangleRounded(
          {(float)panelX, (float)panelY, (float)panelW, (float)panelH}, 0.12f,
          20, panelColor);

      // Weather Panel Content
      DrawText("WEATHER SYSTEM", panelX + 20, panelY + 15, 18, text);
      DrawText(TextFormat("DAY %d", weatherSystem.getCurrentDay()),
               panelX + panelW - 120, panelY + 17, 15, label);
      DrawLine(panelX + 10, panelY + 40, panelX + panelW - 10, panelY + 40,
               line);

      // Dividers
      DrawLineEx({(float)(panelX + 180), (float)(panelY + 50)},
                 {(float)(panelX + 180), (float)(panelY + 160)}, 2, line);
      DrawLineEx({(float)(panelX + 360), (float)(panelY + 50)},
                 {(float)(panelX + 360), (float)(panelY + 160)}, 2, line);
      DrawLineEx({(float)(panelX + 510), (float)(panelY + 50)},
                 {(float)(panelX + 510), (float)(panelY + 160)}, 2, line);

      // WEATHER
      DrawText(weatherSystem.getCurrentWeather().c_str(), panelX + 30,
               panelY + 70, 28, text);
      DrawText("Current Condition", panelX + 30, panelY + 105, 13, label);
      ...
      DrawText(seasonName.c_str(), panelX + 200, panelY + 70, 24, seasonCol);
      DrawText("Season", panelX + 200, panelY + 100, 13, label);
      DrawText(TextFormat("%d days left",
                          season->getLength() - weatherSystem.getCurrentDay()),
               panelX + 200, panelY + 125, 16, text);

      // GROWTH MULTIPLIER
      float growth = season->getGrowthMultiplier();
      DrawText(TextFormat("%.1fx", growth), panelX + 390, panelY + 70, 28,
               green);
      DrawText("Growth Rate", panelX + 390, panelY + 105, 13, label);

      // Progress bar for growth
      DrawRectangleRounded(
          {(float)(panelX + 390), (float)(panelY + 130), 100, 6}, 0.5f, 5,
          barBg);
      DrawRectangleRounded({(float)(panelX + 390), (float)(panelY + 130),
                            100 * (growth / 2.0f), 6},
                           0.5f, 5, green);

      // NEXT DAY TIMER
      float timeLeft = dayLength - dayTimer;
      DrawText(TextFormat("%.1fs", timeLeft), panelX + 540, panelY + 70, 26,
               text);
      DrawText("Next Day", panelX + 540, panelY + 105, 13, label);

      // Time bar
      DrawRectangleRounded(
          {(float)(panelX + 540), (float)(panelY + 130), 80, 6}, 0.5f, 5,
          barBg);
      DrawRectangleRounded({(float)(panelX + 540), (float)(panelY + 130),
                            80 * (dayTimer / dayLength), 6},
                           0.5f, 5, blue);
