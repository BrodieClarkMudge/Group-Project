                 ...
                 int areaWidth, int gridHeight, int offsetX, int offsetY)
{
    ...
    for (auto& tile : tiles) {
        oldTiles.push_back({
            tile.type,
            std::move(tile.animal),
            std::move(tile.crop),
            tile.cropTimer,
            tile.waterCharges,
            tile.wateredGlow,
            tile.waterDecayTimer,
            tile.ripeTimer
        });
    }
...
void Tile::setCrop(std::unique_ptr<Crop> newCrop) {
...
void Tile::removeCrop() {
    crop.reset();
}

void Tile::setWateredGlow(float glow) {
    wateredGlow = glow;
}

float Tile::getWateredGlow() {
    return wateredGlow;
}

void Tile::setWaterDecayTimer(float timer) {
    waterDecayTimer = timer;
}

float Tile::getWaterDecayTimer() {
    return waterDecayTimer;
}

void Tile::setCropTimer(float timer) {
    cropTimer = timer;
}

float Tile::getCropTimer() {
    return cropTimer;
}

void Tile::setWaterCharges(int charges) {
    waterCharges = charges;
}

int Tile::getWaterCharges() {
    return waterCharges;
}

void Tile::setRipeTimer(float timer) {
    ripeTimer = timer;
}

float Tile::getRipeTimer() {
    return ripeTimer;
}